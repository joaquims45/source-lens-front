...
selected symbol centered, callees below. Each row scrolls independently: a
well-tested symbol can have dozens of test-fixture callers, and the
selected symbol must stay visible regardless. Every caller/callee card
shows resolved vs. ambiguous with its confidence and is clickable both to
re-center the trace on it (drilling deeper into the call chain) and to
jump to its source in Files.

### File explorer and code viewer

A tree built from the flat file list, opening the selected file's real
content in Monaco (read-only, syntax-highlighted by language). A citation
or trace card link (`?path=&start=&end=`) resolves against the loaded file
list, opens the right file and highlights the cited line range.

### Insights

Real language/stat/capability data already returned by
`GET /analyses/{id}` — language breakdown, file/symbol/chunk counts, search
capability status — never invented client-side.
...
There is no component test suite yet — the app was verified with Playwright
smoke runs through the full flow (submit → live progress → workspace →
files/Monaco → insights → architecture graph → dependency trace) against
the real backend, which is how three real bugs were caught and fixed: a
routing bug (sidebar links nesting under the active tab instead of
...
cards. Component/integration tests are a natural next step.
